...
describe('request validation', () => {
  it('rejects an invalid JSON body with 400', async () => {
    const response = await sendRequest('/api/send-email', { body: '{not json' });
    expect(response.status).toBe(400);
  });

  it('rejects a request with missing fields', async () => {
    const response = await sendRequest('/api/send-email', { body: { email: SENDER_EMAIL } });
    expect(response.status).toBe(400);
  });
...
describe('server configuration', () => {
  it('returns 500 when secrets are missing', async () => {
    ...
    const response = await sendRequest('/api/send-email', { body: validBody(), env });
...
describe('encryption and delivery', () => {
  it('encrypts the message and sends it to the recipient via Resend', async () => {
    fetchMock.mockClear();
    const response = await sendRequest('/api/send-email', { body: validBody() });
    expect(response.status).toBe(200);
    ...
    const decrypted = await openpgp.decrypt({
      ...
      decryptionKeys: await openpgp.readPrivateKey({ armoredKey: testPrivateKey })
    });
  ...
  it('sets reply_to to the sender email and omits the attachment when no key is given', async () => {
    fetchMock.mockClear();
    await sendRequest('/api/send-email', { body: validBody({ senderPublicKey: '' }) });
    const [, init] = fetchMock.mock.calls[0];
    const payload = JSON.parse(init.body);
...
describe('rate limiting', () => {
  it('rejects a sixth request from the same IP within an hour', async () => {
    ...
    fetchMock.mockClear();
    for (let i = 0; i < 5; i++) {
      const response = await sendRequest('/api/send-email', { body: validBody(), ip });
      expect(response.status).toBe(200);
    }
    const blocked = await sendRequest('/api/send-email', { body: validBody(), ip });
